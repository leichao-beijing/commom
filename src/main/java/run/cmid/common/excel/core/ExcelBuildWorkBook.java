...
public class ExcelBuildWorkBook<T> implements ExcelBuild<T> {
    ...
    public ExcelBuildWorkBook(int sheetMaxRow, Class<T> classes, HeadModel mode,
    ...
    private HashMap<String, DataArray<Object, FieldDetail>> buildRowMap(Row row,
    ...
    private void buildCellMap(HashMap<String, DataArray<Object, FieldDetail>> data, String name, CellType type,
                              Cell cell, FieldDetail detail) {
        switch (type) {
            case _NONE:
                data.put(name, new DataArray<Object, FieldDetail>(null, detail));
                break;
            case BLANK:
                ...
                break;
            case BOOLEAN:
                data.put(name, new DataArray<Object, FieldDetail>(cell.getBooleanCellValue(), detail));
                break;
            case ERROR:
                data.put(name, new DataArray<Object, FieldDetail>(null, detail));
                break;
            case FORMULA:
                ...
                break;
            case NUMERIC:
                data.put(name, new DataArray<Object, FieldDetail>(cell.getNumericCellValue(), detail));
                break;
            case STRING:
                data.put(name, new DataArray<Object, FieldDetail>(cell.toString(), detail));
                break;
        }
    }
     ...
     * @throws ConverterExcelException
     */
    private ExcelResult<T> build(Row row, List<CompareResponse<FieldDetail, String>> list, Class<T> classes) {
        T out = ReflectUtil.newInstance(classes);
        HashMap<String, DataArray<Object, FieldDetail>> map = buildRowMap(row, list);
        ...
        for (CompareResponse<FieldDetail, String> compareResponse : list) {
            ...
            // Cell cell = row.getCell(compareResponse.getDesIndex());
            FieldDetail detail = compareResponse.getSrcData();
            DataArray<Object, FieldDetail> value = map.get(detail.getFieldName());
     ...
     * 生成list对象
     */
    private void invokeValue(Object out, String setFunctionValue, Object value, FieldDetail fieldDetail) {
        if (fieldDetail.getType() == FieldDetailType.LIST) {
